...
import srez_model
import srez_train
from srez_train import _summarize_progress,_summarize_progress_single
...
tf.app.flags.DEFINE_string('mydataset', 'mydataset',
                           "Path to my dataset directory.")
...
def _demo(filenames):
    ...
    with tf.Session() as sess:
        ...
        [gene_minput, gene_moutput, gene_output, gene_var_list, disc_real_output, disc_fake_output, disc_var_list] = init_model(sess)
...
def init_model(sess):
	all_filenames = prepare_dirs()
	test_filenames  = all_filenames[:]
	sample_features,sample_labels  = srez_input.setup_inputs(sess,all_filenames)
	filename = 'checkpoint_new.txt'
	filename = os.path.join(FLAGS.checkpoint_dir, filename)
	[gene_minput, gene_moutput,
     gene_output, gene_var_list,
     disc_real_output, disc_fake_output, disc_var_list] = \
     srez_model.create_model(sess,  sample_features, sample_labels)
	saver = tf.train.Saver()
	saver.restore(sess, filename)
	return [gene_minput, gene_moutput, gene_output, gene_var_list,  disc_real_output, disc_fake_output, disc_var_list]
