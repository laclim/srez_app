...
def _summarize_progress_single(train_data, feature, gene_output, suffix, max_samples=1):
    td = train_data

    size = [gene_output.shape[1], gene_output.shape[2]]
    ...
    image   = tf.concat(axis=2, values=[clipped])
...
def train_model(train_data):
    ...
    while not done:
        ...
        ops = [td.gene_minimize, td.disc_minimize, td.gene_loss, td.disc_real_loss, td.disc_fake_loss,merged]
        _, _, gene_loss, disc_real_loss, disc_fake_loss,summary = td.sess.run(ops, feed_dict=feed_dict)
